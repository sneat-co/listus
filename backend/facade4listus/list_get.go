...
// Returns a not-found error (detectable via dal.IsNotFound) if the list is absent.
	...
	err = dal4listus.RunListWorker(ctx, request,
		func(ctx facade.ContextWithUser, tx dal.ReadwriteTransaction, params *dal4listus.ListWorkerParams) (err error) {
			if !params.List.Record.Exists() {
				return dal.NewErrNotFoundByKey(params.List.Record.Key(), dal.ErrRecordNotFound)
			}
			list = params.List
			return nil
		},
